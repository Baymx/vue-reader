...
<script>
  ...
  export default {
    ...
    created(){
      ...
      //加载时从localStorage中获取去所有数据阅读进度
      const localBookReaderInfo = localEvent.StorageGetter('booksReadInfo');
      let  id = this.$route.params.id;
      if (localBookReaderInfo　&&　localBookReaderInfo[id]){
        this.booksReadInfo = localEvent.StorageGetter('booksReadInfo');
    ...
    methods:{
      ...
      getData(id,chapter){
        this.loading = true;
        axios.get(`${this.common.api}/book?book=${id}&id=${chapter}`)
          .then((res)=>{
            console.log(res)
            this.loading = false;
      ...
      saveBooksInfo(){
        ...
        localEvent.StorageSetter("booksReadInfo",this.booksReadInfo);
      },
    },
    computed:{
      ...mapState([
